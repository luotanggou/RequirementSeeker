...
- 导出、打印或在采集器代码中直接读取 Cookie、Token、请求头和密码；复用用户日常浏览器配置；
- 自动生成语义金标、模型分析或机会创建；
...
1. 使用可见 Playwright 浏览器。默认上下文不持久化；用户于 2026-09-12 明确要求后，允许选择已安装的 Chrome 或 Edge，并仅在专用本地配置目录中持久化平台登录态。
2. 允许监听页面自身发出的评论接口响应，但只解析响应正文；采集器不得通过 API 读取、序列化、导出或输出请求头、Cookie、Token 或浏览器配置。持久模式下的专用 profile 仅由浏览器自身维护。
...
11. 持久登录模式不得打开、复制或修改用户日常 Chrome/Edge profile。每个平台与浏览器使用 `.local-data/m2-real/browser-profiles/<platform>/<browser>/` 下的独立 profile；浏览器可在其中保存 Cookie 和会话凭据，但采集器不得调用 Cookie、storage-state 或请求头读取/导出接口，也不得把 profile 内容写入日志、报告、备份或数据产物。
...
| BrowserSession | 启动非无头临时 Chromium，或使用平台隔离的专用 Chrome/Edge profile，等待用户登录并提供页面事件 | 读取/导出登录凭据、复用日常浏览器 profile 或隐藏自动化特征 |
...
1. CLI 创建运行 ID 和本地运行目录。默认启动非无头 Chromium 临时上下文；显式启用持久登录时，启动已安装的 Chrome/Edge，并把 user-data-dir 固定到输出根内的平台专用目录。
...
10. 关闭浏览器上下文。临时模式不写 storage state 或用户数据目录；持久模式保留浏览器自行维护的专用 profile，但采集器不读取、序列化或复制其中内容。
